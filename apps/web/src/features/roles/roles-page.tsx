...
export function RolesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Roles"
        description="Basic role metadata management. Permission builder is not implemented yet."
        actions={<Button onClick={() => setOpen(true)}>Create role</Button>}
      />
            ...
            <Link key={role.id} href={`/roles/${role.id}`}>
              <Card className="p-5 transition hover:-translate-y-0.5 hover:shadow-pop">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-cream-100 px-2 py-1 text-[11px] text-ink-600 ring-1 ring-ink-200">
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create custom role">
      <form onSubmit={submit} className="space-y-4">
        <Field
          label="Role name"
          hint="3-50 lowercase letters, numbers, or underscores."
        >
          <input
